...
namespace RobotView
{
    public partial class RunLine : UserControl
    {
        ...
        private void btnKeyboardLine_Click(object sender, EventArgs e)
        {
            NumberKeyboard nk = new NumberKeyboard();
            if (nk.ShowDialog() == DialogResult.OK)
            {
                decimal number = (decimal)nk.Number;
                if(number >= upDownLineLength.Minimum && number <= upDownLineLength.Maximum)
                    this.upDownLineLength.Value = number;
            }
        }
    }
}